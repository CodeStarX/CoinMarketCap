...
class CryptoVolumeFilterPagingSource(
    private val serviceImp: ServiceImp,
    private val volumeParams: CryptosVolumeFilterUseCase.Params
    ...
    override suspend fun load(params: LoadParams<Int>): LoadResult<Int, CryptoCurrency> {
        ...
        return try {
            val response = serviceImp.getCryptosByVolumeFilter(start = page.toString(), min = volumeParams.volumeMin, max = volumeParams.volumeMax)
